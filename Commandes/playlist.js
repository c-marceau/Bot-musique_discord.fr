...
module.exports = {
  ...
  cooldown: 5,
  aliases: ["pl"],
  description: "Mets une playlist sur youtube",
  async execute(message, args) {
    ...
    if (!permissions.has("CONNECT"))
      return message.reply("Impossible de se connecter au canal vocal, autorisations manquantes");
    if (!permissions.has("SPEAK"))
      return message.reply("Je ne peux pas parler dans ce canal vocal, assurez-vous que j'ai les autorisations appropriées!");
    ...
    if (urlValid) {
      try {
        ...
        console.error(error);
        return message.reply("Liste de lecture introuvable :(").catch(console.error);
      }
    } else if (scdl.isValidUrl(args[0])) {
      if (args[0].includes("/sets/")) {
        message.channel.send("⌛ récupérer la playlist...");
    ...
    if (!serverQueue) {
      ...
      try {
        ...
        await channel.leave();
        return message.channel.send(`Impossible de rejoindre la chaîne : ${error.message}`).catch(console.error);
      }
    }
  }
};